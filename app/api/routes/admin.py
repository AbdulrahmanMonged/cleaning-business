...
async def change_user_role(
    ...
    updated_user = await change_role(user=user, target_user=target_user, db=db, user_id=user_id)
    if updated_user is None:
        raise HTTPException(
            status=status.HTTP_404_NOT_FOUND, detail="Could not find user"
        )
    return UserPublic.model_validate(updated_user)


@router.get("/{user_id}")
async def get_user_by_id(user_id: int, db: db_dependency, role: role_dependency[Roles.ADMIN]):
